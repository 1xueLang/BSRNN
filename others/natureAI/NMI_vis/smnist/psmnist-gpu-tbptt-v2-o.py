...
class RNN_custom(nn.Module):
    def __init__(self, input_size, stride, hidden_dims, output_size,DC_f='mem'):
        ...
        self.i2h = nn.Linear(620, 1024)
        self.h2h = nn.Linear(1024, 1024)
        self.h2d = nn.Linear(self.r1_dim, self.r2_dim)
        self.d2d = nn.Linear(self.r2_dim, self.r2_dim)
        self.dense1 = nn.Linear(1024, 256)
        self.d2o = nn.Linear(256, 11)

        self.tau_adp_r1 = nn.Parameter(torch.Tensor(1024))
        ...
        self.tau_m_r1 = nn.Parameter(torch.Tensor(1024))
    ...
    def initial_neuron_states(self,batch_size):
        self.r1_mem = self.r1_spike = torch.rand(batch_size, 1024).cuda()*self.b_j0
        ...
        self.d2o_mem = torch.rand(batch_size, 11).cuda()*self.b_j0
        self.d2o_spike = self.output_sumspike = torch.zeros(batch_size, 11).cuda()
...
def train(model, num_epochs,train_loader,test_loader,file_name,MyFile):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(tqdm(train_loader)):
            ...
            labels = labels.long().squeeze(dim=1).to(device)
