...
class ExcitatoryInhibitoryBlock(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor) -> List[torch.Tensor]:
        inputs = nn.functional.dropout(inputs, p=0.5, training=self.training)
        h = self.synapse_ih(inputs)
        # h = nn.functional.dropout(h, p=0.2, training=self.training)
        if self.h_last is not 0: 
            h += self.synapse_hh(nn.functional.dropout(self.h_last, p=0.2, training=self.training))
            # h += nn.functional.dropout(self.synapse_hh(self.h_last), p=0.2, training=self.training)
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(inputs.shape[1]):
            ...
            for rnn_layer in self.rsnns[1:]:
                se, si = rnn_layer(se)
            se = nn.functional.dropout(se, p=0.3, training=self.training)
            h = self.fc_out(se)
            # h = nn.functional.dropout(h, p=0.2, training=self.training)
            s = self.neuron(h)
            out.append(s)
