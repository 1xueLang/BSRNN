...
class RNN_custom(nn.Module):
    ...
    def initial_neuron_states(self,batch_size):
        ...
        self.r1_mem = self.r1_spike = torch.rand(batch_size, self.r1_dim) * self.b_j0
        self.r2_mem = self.r2_spike = torch.rand(batch_size, self.r2_dim)* self.b_j0
        self.d1_mem = self.d1_spike = torch.rand(batch_size, self.d1_dim)* self.b_j0
        self.d2o_mem = torch.rand(batch_size, output_dim)* self.b_j0
        self.d2o_spike = self.output_sumspike = torch.zeros(batch_size, output_dim)
